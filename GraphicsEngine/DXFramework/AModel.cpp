...
void AModel::computeBoundingBox()
{
	  // Initialize min and max points
    //boundingBoxMin = XMFLOAT3(std::numeric_limits<float>::max(),
...
void AModel::importModel(const std::string& pFile)
{
	...
	//delete indices;
	//indices = 0;
	vertexCount = (int)vertices.size(); indexCount = (int)indices.size();
