...
RenderTexture::RenderTexture(ID3D11Device* device, int ltextureWidth, int ltextureHeight, float screenNear, float screenFar)
{
	...
	depthShaderResourceViewDesc.Texture2D.MostDetailedMip = 0;
 	...
 	result = device->CreateShaderResourceView(depthStencilBuffer, &depthShaderResourceViewDesc, &depthResourceView);
...
void RenderTexture::setAsRenderTarget(ID3D11DeviceContext* deviceContext)
{
...
ID3D11Texture2D* RenderTexture::getDepthTexture()
{
	return depthStencilBuffer;
}

ID3D11ShaderResourceView* RenderTexture::getDepthShaderResourceView()
{
