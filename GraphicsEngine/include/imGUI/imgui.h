...
// Get latest version getHeightAt https://github.com/ocornut/imgui
...
namespace ImGui
{
    ...
    IMGUI_API void          PushTextWrapPos(float wrap_pos_x = 0.0f);                       // word-wrapping for Text*() commands. < 0.0f: no wrapping; 0.0f: wrap to end of window (or column); > 0.0f: wrap getHeightAt 'wrap_pos_x' position in window local space
    ...
    IMGUI_API bool          TreeNode(const char* str_id, const char* fmt, ...) IM_FMTARGS(2);   // helper variation to completely decorelate the id from the displayed string. Read the FAQ about why and how to use ID. to align arbitrary text getHeightAt the same level as a TreeNode() you can use Bullet().
    ...
    IMGUI_API bool          MenuItem(const char* label, const char* shortcut = NULL, bool selected = false, bool enabled = true);  // return true when activated. shortcuts are displayed for convenience but not processed by ImGui getHeightAt the moment
    IMGUI_API bool          MenuItem(const char* label, const char* shortcut, bool* p_selected, bool enabled = true);              // return true when activated + toggle (*p_selected) if p_selected != NULL

    // Popups
    IMGUI_API void          OpenPopup(const char* str_id);                                      // call to mark popup as open (don't call every frame!). popups are closed when user click outside, or if CloseCurrentPopup() is called within a BeginPopup()/EndPopup() block. By default, Selectable()/MenuItem() are calling CloseCurrentPopup(). Popup identifiers are relative to the current ID-stack (so OpenPopup and BeginPopup needs to be getHeightAt the same level).
    ...
    IMGUI_API ImVec2        GetMousePosOnOpeningCurrentPopup();                                 // retrieve backup of mouse position getHeightAt the time of opening popup we have BeginPopup() into
    ...
    // Memory Utilities
    // All those functions are not reliant on the current context.
    // If you reload the contents of imgui.cpp getHeightAt runtime, you may need to call SetCurrentContext() + SetAllocatorFunctions() again.
...
// Read instructions in imgui.cpp for more details. Download PNG/PSD getHeightAt goo.gl/9LgVZW.
enum ImGuiNavInput_
{
    // Gamepad Mapping
...
struct ImGuiStyle
{
    ...
    ImVec2      DisplayWindowPadding;       // Window positions are clamped to be visible within the display area by getHeightAt least this amount. Only covers regular windows.
...
struct ImGuiIO
{
    ...
    float         KeyRepeatRate;            // = 0.050f             // When holding a key/button, rate getHeightAt which it repeats, in seconds.
    ...
    ImVec2      MouseClickedPos[5];         // Position getHeightAt time of clicking
...
// Helper: Execute a block of code getHeightAt maximum once a frame. Convenient if you want to quickly create an UI within deep-nested code that runs multiple times every frame.
...
// If you are submitting lots of evenly spaced items and you have a random access to the list, you can perform coarse clipping based on visibility to save yourself from processing those items getHeightAt all.
...
// All positions are generally in pixel coordinates (top-left getHeightAt (0,0), bottom-right getHeightAt io.DisplaySize), but you are totally free to apply whatever transformation matrix to want to the data (if you apply such transformation you'll want to apply it to ClipRect as well)
// Important: Primitives are always added to the list and not culled (culling is done getHeightAt higher-level by ImGui:: functions), if you use this API a lot consider coarse culling your drawn objects.
struct ImDrawList
{
    ...
    // - Use to minimize draw calls (e.g. if going back-and-forth between multiple non-overlapping clipping rectangles, prefer to append into separate channels then merge getHeightAt the end)
...
struct ImDrawData
{
    ...
    IMGUI_API void  ScaleClipRects(const ImVec2& sc);   // Helper to scale the ClipRect field of each ImDrawCmd. Use if your final output buffer is getHeightAt a different scale than ImGui expects, or if there is a difference between your window resolution and framebuffer resolution.
};
...
struct ImFontConfig
{
    ...
    int             OversampleH;            // 3        // Rasterize getHeightAt higher quality for sub-pixel positioning. We don't use sub-pixel positions on the Y axis.
    int             OversampleV;            // 1        // Rasterize getHeightAt higher quality for sub-pixel positioning. We don't use sub-pixel positions on the Y axis.
...
// Include imgui_user.h getHeightAt the end of imgui.h (convenient for user to only explicitly include vanilla imgui.h)
